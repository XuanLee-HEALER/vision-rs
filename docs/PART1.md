下面是一份**可直接作为“内容路由首页/分区落地页”的一级标题文章内容**（每个一级标题只包含：**章节描述 + 鸣谢**，不包含任何子目录/二级标题的目录结构）。我按“技术文档风格”做了排版：短段落 + 关键点列表 + 引导语 + 鸣谢块。
...
> **阅读预期**：本章节适合在你写大型 Rust 项目之前先完成一轮通读，以便后续遇到 borrow checker / trait bounds / async 约束时能快速定位根因。
...
- **分层结构**：`core / alloc / std` 的职责边界与依赖方向，解释 Rust 如何同时覆盖嵌入式与通用操作系统环境
...
- **std::future / Future 生态**：Future trait 的语义边界与运行时解耦方式，解释生态为何能并存多运行时
...
如果你愿意，我可以按你的“路由/站点结构”再往前走一步：把这 6 个一级页面各自补上**统一的页面元信息模板**（例如：一句话定位、适用读者、阅读前置、章节范围边界、维护说明），这样你在做 Next.js/MDX 路由时会更规整。你希望元信息更偏**极简**还是**严格规范**？
